...
void Particle_Helpers::Contact(Particle * Body_A, const unsigned int Num_Particles_A,Particle * Body_B, const unsigned int Num_Particles_B, const double h_In) {
  ...
  const double h = 2*h_In;                      // Contact force support radius          : unitless
  const double h_squared = h*h;                 // Square of support radius              : unitless
  unsigned int i,j;
  double V_i, V_j, K_V_i;                                                      //        : mm^3
  double Mag_r_ij;                                                             //        : mm
  ...
  for(i = 0; i < Num_Particles_A; i++) {
    ...
    for(j = 0; j < Num_Particles_B; j++) {
      ...
      if(Vector_Dot_Product(r_ij, r_ij) < h_squared) {
        ...
        Grad_W = (-3*(Particle::Shape_Function_Amp/64.)*((h - Mag_r_ij)*(h - Mag_r_ij))/Mag_r_ij)*(r_ij);    // mm Vector
